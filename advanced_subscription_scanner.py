#!/usr/bin/env python3
"""
Advanced subscription scanner with comprehensive keyword detection and JSON export.
...
try:
    ...
    print("Warning: Enhanced detectors not found. Using basic payment extraction.")
...
# Comprehensive subscription keywords (111 total)
...
class AdvancedSubscriptionScanner:
    """Advanced scanner with comprehensive keyword detection"""
    ...
    def _is_spam(self, subject: str, sender: str) -> bool:
        """Check if email is likely spam - basic check for when enhanced detection not available"""
        if ENHANCED_DETECTION:
            # Enhanced detector will handle this better
            return False
        
        text = f"{subject} {sender}".lower()
        return any(re.search(pattern, text) for pattern in SPAM_PATTERNS)
    
    def scan_emails(self, days_back: int = 7300, dry_run: bool = False) -> Dict:  # Default 20 years
        ...
        for i in range(0, len(SUBSCRIPTION_KEYWORDS), batch_size):
            ...
            if has_subjects_table:
                # Normalized database - FIXED recipient join
                keyword_conditions = []
                for keyword in batch_keywords:
                    escaped = keyword.replace("'", "''")
                    keyword_conditions.append(f"LOWER(s.subject) LIKE LOWER('%{escaped}%')")
        ...
        # Filter out spam/fraud
        filtered_results = []
        spam_count = 0
        fraud_count = 0
        
        for result in unique_results:
            _, subject, _, sender, _ = result
            ...
            if ENHANCED_DETECTION:
                ...
                is_fraud, fraud_reasons = EnhancedFraudDetector.is_likely_fraud(
                    sender or '', subject or '', amount, frequency
                )
            ...
            if filtered_results:
                ...
                if 'recipients' not in tables_found or 'addresses' not in tables_found:
                    ...
                    if message_link_col_rec and address_data_col_rec and (not address_data_is_fk or email_text_col_addr):
                        print(f"   🔍 Attempting to fetch recipient details...")
                        ...
                        current_where_conditions = []
                        if type_col_rec:
                            current_where_conditions.append(f"r.{type_col_rec} = 0")

                        all_message_ids_from_results = [str(r[0]) for r in filtered_results]
                        batch_size = 500
                        ...
                        for i in range(0, len(all_message_ids_from_results), batch_size):
                            ...
                            batch_where_conditions = list(current_where_conditions)
                            batch_where_conditions.append(f"r.{message_link_col_rec} IN ({batch_ids_str})")
                            
                            final_recipient_query = f"{' '.join(recipient_query_parts)} WHERE {' AND '.join(batch_where_conditions)}"
        ...
        if ENHANCED_DETECTION:
            ...
            print(f"✅ Filtered {fraud_count} fraudulent/phishing emails")
        ...
        # Process results
        self._process_results(filtered_results, recipients_map)
        ...
        if COMMON_STRUCTURES_AVAILABLE:
            ...
            self.scan_results = ScanResults(
                ...
                scan_parameters={
                    ...
                    'spam_filtered': fraud_count if ENHANCED_DETECTION else spam_count
                },
    ...
    def _process_results(self, emails: List[tuple], recipients_map: Dict[int, List[str]]) -> None:
        """Process email results with advanced categorization and enhanced detection"""
        ...
        for email in emails:
            ...
            if COMMON_STRUCTURES_AVAILABLE:
                ...
                # Extract financial information with enhanced extraction
                if ENHANCED_DETECTION:
                    amount, currency, frequency = EnhancedPaymentExtractor.extract_amount_and_frequency(subject or '')
                else:
                ...
                if amount:
                    ...
                    if frequency and ENHANCED_DETECTION:
    ...
    def _extract_amount_advanced(self, text: str) -> Tuple[Optional[float], Optional[str]]:
        ...
        if ENHANCED_DETECTION:
            amount, currency, _ = EnhancedPaymentExtractor.extract_amount_and_frequency(text)
    ...
    def _extract_frequency(self, text: str) -> Optional[str]:
        """Extract subscription frequency using enhanced or basic extractor"""
        if ENHANCED_DETECTION:
            return EnhancedPaymentExtractor.extract_frequency(text)
        else:
    ...
    def _find_duplicates(self) -> List[DuplicateSubscription]:
        """Find duplicate subscriptions with fuzzy matching"""
        duplicates = []
        
        if not COMMON_STRUCTURES_AVAILABLE:
            return duplicates
        
        # First pass: exact company name matches
        for company, sub in self.subscriptions.items():
            if len(sub.accounts) > 1:
                monthly_waste = 0
                if sub.avg_amount:
                    # avg_amount is already monthly equivalent if enhanced detection is on
    ...
    def display_results(self, show_all: bool = False) -> None:
        ...
        if COMMON_STRUCTURES_AVAILABLE and self.scan_results:
            ...
            if financial['duplicate_monthly_waste'] > 0:
                ...
                print(f"   Found: {summary['duplicate_count']} services on multiple accounts")
                print(f"   Monthly waste: {format_currency(financial['duplicate_monthly_waste'])}")
                print(f"   Annual waste: {format_currency(financial['duplicate_annual_waste'])}")
            ...
            for account, count in sorted(account_counts.items(), key=lambda x: x[1], reverse=True)[:5]:
                print(f"   {account}: {count} subscriptions")
        ...
        for i, (company, sub) in enumerate(sorted_subs[:limit], 1):
            ...
            print(f"   👤 {len(sub.accounts)} account(s): {', '.join(list(sub.accounts)[:2])}")
            ...
            if hasattr(sub, 'metadata') and 'full_senders' in sub.metadata:
                ...
                if senders:
                    print(f"   📨 From: {list(senders)[0][:60]}..." if len(list(senders)[0]) > 60 else f"   📨 From: {list(senders)[0]}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Advanced subscription scanner with comprehensive detection'
    )
    parser.add_argument(
        '--days', 
        type=int, 
